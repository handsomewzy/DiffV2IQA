...
from models.maniqa_diff2 import MANIQA # 加载新模型
...
if __name__ == '__main__':
    ...
    config = Config({
        ...
        # TID2013
        "TID2013_path": "/mnt/workspace/workgroup/zhaoyang.wzy/tid2013",
        "TID2013_label": "/mnt/workspace/workgroup/zhaoyang.wzy/tid2013/mos_with_names.txt",

        # LIVE
        "LIVE_path": "/mnt/workspace/workgroup/zhaoyang.wzy/databaserelease2",
        "LIVE_label": "/mnt/workspace/workgroup/zhaoyang.wzy/MANIQA/data/LIVE_lable.txt",

        # CSIQ
        "CSIQ_path": "/mnt/workspace/workgroup/zhaoyang.wzy/csiq",
        "CSIQ_label": "/mnt/workspace/workgroup/zhaoyang.wzy/MANIQA/data/AbpathCSIQ_label.txt",
    ...
    # 进行交叉数据集的验证，根据设定再读取对应的验证数据集
    if config.cross_dataset_name == 'TID2013':
        from data.TID2013 import TID2013Folder
        train_name1, val_name1 = split_dataset_TID2013(
            txt_file_name=config.TID2013_label,
            split_seed=config.split_seed
        )
        dis_train_path1 = config.TID2013_path
        dis_val_path1 = config.TID2013_path
        label_train_path1 = config.TID2013_label
        label_val_path1 = config.TID2013_label
        TestDataset = TID2013Folder
    elif config.cross_dataset_name == 'LIVE':
        from data.LIVE import LIVEFolder
        train_name1, val_name1 = split_dataset_live(
            txt_file_name=config.LIVE_label,
            split_seed=config.split_seed
        )
        dis_train_path1 = config.LIVE_path
        dis_val_path1 = config.LIVE_path
        label_train_path1 = config.LIVE_label
        label_val_path1 = config.LIVE_label
        TestDataset = LIVEFolder
    elif config.cross_dataset_name == 'CSIQ':
        from data.CSIQ import CSIQFolder
        train_name1, val_name1 = split_dataset_live(
            txt_file_name=config.CSIQ_label,
            split_seed=config.split_seed
        )
        dis_train_path1 = config.CSIQ_path
        dis_val_path1 = config.CSIQ_path
        label_train_path1 = config.CSIQ_label
        label_val_path1 = config.CSIQ_label
        TestDataset = CSIQFolder

    elif config.cross_dataset_name == 'LiveC':
        from data.LIVEC import LiveCFolder
        train_name1, val_name1 = split_dataset_live(
            txt_file_name=config.LiveC_label,
            split_seed=config.split_seed
        )
        dis_train_path1 = config.LiveC_path
        dis_val_path1 = config.LiveC_path
        label_train_path1 = config.LiveC_label
        label_val_path1 = config.LiveC_label
        TestDataset = LiveCFolder   
    else:
        pass
    ...
    val_dataset = TestDataset(
        dis_path=dis_val_path1,
        txt_file_name=label_val_path1,
        list_name=val_name1,
        transform=transforms.Compose([RandCrop(patch_size=config.crop_size),
    ...
    # 加载训练好的模型
    # net.load_state_dict(torch.load("/mnt/workspace/workgroup/zhaoyang.wzy/MANIQA/output/models/Pipal_train/LiveC_dviqa_s20/epoch27.pt"))
