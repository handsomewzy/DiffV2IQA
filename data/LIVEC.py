import torch.utils.data as data
from PIL import Image
import os
import os.path
import scipy.io
import numpy as np
import csv
from openpyxl import load_workbook
import cv2
import random
import torch
import torchvision

import os
import torch
import numpy as np
import cv2
import torch.nn.functional as F
from torchvision import transforms
from glob import glob

class LiveCFolder(data.Dataset):
    def __init__(self, dis_path, txt_file_name, list_name, transform, keep_ratio):
        super(LiveCFolder, self).__init__()
        self.dis_path = dis_path
        self.txt_file_name = txt_file_name
        self.transform = transform

        dis_files_data, score_data = [], []
        with open(self.txt_file_name, 'r') as listFile:
            for line in listFile:
                dis, score = line.split()
                if dis in list_name:
                    score = float(score)
                    dis_files_data.append(dis)
                    score_data.append(score)
        # reshape score_list (1xn -> nx1)
        score_data = np.array(score_data)
        score_data = self.normalization(score_data)
        score_data = list(score_data.astype('float').reshape(-1, 1))

        self.data_dict = {'d_img_list': dis_files_data, 'score_list': score_data}

    
    def __getitem__(self, idx):
        d_img_name = self.data_dict['d_img_list'][idx]
        d_img = cv2.imread(d_img_name, cv2.IMREAD_COLOR)
        # 读取扩散模型生成的模型
        basename = os.path.basename(d_img_name)
        dirname = os.path.dirname(d_img_name)
        dir_path = dirname.replace('Images', 'diffusion_imgs_pipal')
        diff_0_path = os.path.join(dir_path, basename + '_0.png')
        diff_1_path = os.path.join(dir_path, basename + '_1.png')
        diff_2_path = os.path.join(dir_path, basename + '_2.png')

        d0 = cv2.imread(diff_0_path, cv2.IMREAD_COLOR)
        d1 = cv2.imread(diff_1_path, cv2.IMREAD_COLOR)
        d2 = cv2.imread(diff_2_path, cv2.IMREAD_COLOR)

        d_img, d0, d1, d2 = self.preprocess(d_img), self.preprocess(d0), self.preprocess(d1), self.preprocess(d2)
        score = self.data_dict['score_list'][idx]
        # print(d_img.shape,d0.shape)
        # if self.transform:
        #     t_d_img = self.transform(d_img)
        #     t_d0 = self.transform(d0)
        #     t_d1 = self.transform(d1)
        #     t_d2 = self.transform(d2)
        # # print(t_d0.shape, d0.shape)

        sample = {
            'd_img_org': d_img,
            'd0': d0,
            'd1': d1,
            'd2': d2,
            'score': score
        }
        sample = self.transform(sample)

        # print(sample)
        # asd
        
        return sample

    def preprocess(self, d_img):
        x = cv2.resize(d_img, (224, 224), interpolation=cv2.INTER_CUBIC)

        # LIVEC 数据大小参差不齐，进行大小匹配
        # new_height = (d_img.shape[0] // 16) * 16
        # new_width = (d_img.shape[1] // 16) * 16
        # x = cv2.resize(d_img, (new_width, new_height), interpolation=cv2.INTER_CUBIC)

        x = cv2.cvtColor(x, cv2.COLOR_BGR2RGB)
        x = np.array(x).astype('float32') / 255
        x = np.transpose(x, (2, 0, 1))   
        return x  

    def normalization(self, data):
        range = np.max(data) - np.min(data)
        return (data - np.min(data)) / range

    def __len__(self):
        return len(self.data_dict['d_img_list'])



# import os
# import scipy.io
# import numpy as np
# from glob import glob

# # 根路径
# root = '/mnt/workspace/workgroup/zhaoyang.wzy/ChallengeDB_release'

# # 获取所有图像的绝对路径
# sr_path = glob(os.path.join(root, 'Images', '**/*.bmp'), recursive=True) + \
#           glob(os.path.join(root, 'Images', '**/*.JPG'), recursive=True)

# # 创建一个映射，将图像的文件名（不包括扩展名）映射到其完整路径
# filename_to_path = {os.path.splitext(os.path.basename(p))[0]: p for p in sr_path}

# # 加载图像路径和MOS
# imgpath = scipy.io.loadmat(os.path.join(root, 'Data', 'AllImages_release.mat'))['AllImages_release'][7:1169]
# mos = scipy.io.loadmat(os.path.join(root, 'Data', 'AllMOS_release.mat'))['AllMOS_release'].astype(np.float32)[0][7:1169]

# # 将图像路径和对应的MOS存储到文本文件中
# with open('livec_labels.txt', 'w') as outFile:
#     for path, label in zip(imgpath, mos):
#         # 从数组中提取字符串元素
#         filename = os.path.splitext(path[0][0])[0] if isinstance(path, np.ndarray) and path.size == 1 else path

#         # 检查sr_path中是否存在对应的图像
#         if filename in filename_to_path:
#             # 获取对应的绝对路径
#             full_path = filename_to_path[filename]
#             # 写入一行，包含图像的完整路径和其MOS，中间以空格间隔
#             outFile.write(f"{full_path} {label}\n")

# # 打印找到的图像数量和总MOS数量
# print(f"Found {len(filename_to_path)} images, with {len(mos)} total MOS scores.")


