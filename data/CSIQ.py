...
class CSIQFolder(data.Dataset):
    def __init__(self, dis_path, txt_file_name, list_name, transform, keep_ratio):
        super(CSIQFolder, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        dir_path = dirname.replace('dst_imgs', 'diffusion_imgs_pipal')
...
# # 获取所有图像的绝对路径
# dis_list = glob('/mnt/workspace/workgroup/zhaoyang.wzy/csiq/dst_imgs/**/*.png', recursive=True)

# # 创建一个映射，将图像名称映射到对应的绝对路径
# name_to_path = {os.path.basename(path): path for path in dis_list}

# # 读取包含图像名称和分数的文件
# with open('/mnt/workspace/workgroup/zhaoyang.wzy/MANIQA/data/csiq_label.txt', 'r') as listFile:
#     # 准备用于存储匹配后结果的列表
#     matched_data = []
...
# # 将匹配后的结果写入新的文本文件
# with open('/mnt/workspace/workgroup/zhaoyang.wzy/MANIQA/data/matched_paths_scores.txt', 'w') as outputFile:
#     # 遍历所有匹配的数据
