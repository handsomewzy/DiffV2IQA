...
class RandCrop(object):
    ...
    def __call__(self, sample):
        ...
        if h == new_h and w == new_w:
            ...
            ret_d0 = d0[:, top: top + new_h, left: left + new_w]
            ret_d1 = d1[:, top: top + new_h, left: left + new_w]
            ret_d2 = d2[:, top: top + new_h, left: left + new_w]
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        d0 = (d0 - self.mean) / self.var
        d1 = (d1 - self.mean) / self.var
        d2 = (d2 - self.mean) / self.var
        sample = {
            'd_img_org': d_img,
            'd0': d0,
            'd1': d1,
...
class RandHorizontalFlip(object):
    ...
    def __call__(self, sample):
        ...
        if prob_lr > 0.5:
            ...
            d0 = np.fliplr(d0).copy()
            d1 = np.fliplr(d1).copy()
            d2 = np.fliplr(d2).copy()
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        d0 = torch.from_numpy(d0).type(torch.FloatTensor)
        d1 = torch.from_numpy(d1).type(torch.FloatTensor)
        d2 = torch.from_numpy(d2).type(torch.FloatTensor)
